...
--
-- Build a real document as superuser, then confirm the unprivileged role
-- still can't write it to a server file, and that superuser can.
--
...
-- (via the htmldoc_test_none block above) -- its success path for
-- superuser was never actually run. Confirm it end-to-end via
...
-- superuser too; confirm success end-to-end via the in-memory (bytea)
...
--
-- pg_htmldoc.whitelist (see the pg_whitelist submodule) adds an optional
-- restriction on top of the superuser check above: even a superuser can be
-- scoped down to specific file:// / http(s):// prefixes. pg_htmldoc.whitelist
-- is registered PGC_SUSET, so a superuser session can enable it with a
-- plain SET -- no reconnect needed, unlike the role-level defaults used for
-- htmldoc_test_none elsewhere in this file.
--

--
-- An exact file:// entry permits only that file.
--
SELECT set_config('pg_htmldoc.whitelist', 'file:///etc/hostname', false);
...
--
SELECT set_config('pg_htmldoc.whitelist', 'file:///etc/ssl/', false);

SELECT htmldoc_addfile('/etc/ssl/openssl.cnf');
SELECT octet_length(convert2pdf()) > 100 AS whitelist_directory_match_pdf_nonempty;
SELECT htmldoc_addfile('/etc/hostname');
SELECT htmldoc_addfile('/etc/ssl/../passwd');

--
-- http(s):// entries are enforced *before* the URL is fetched --
-- pg_whitelist_check_url() runs ahead of htmldoc's file_find() in
-- read_fileurl(), specifically so a disallowed host is never actually
-- contacted. 192.0.2.0/24 is the RFC 5737 documentation range: reserved and
-- guaranteed non-routable, so if the fetch were attempted anyway (i.e. if
-- this ordering regressed) the error below would come back as an internal
-- "!file_find" failure instead of the permission-denied error this checks
-- for -- and it would take a while doing it too, since htmldoc's HTTP
-- client retries with real connect timeouts.
--
SELECT set_config('pg_htmldoc.whitelist', 'https://example.com/', false);

SELECT htmldoc_addurl('http://192.0.2.1/forbidden');

SELECT set_config('pg_htmldoc.whitelist', '', false);

--
-- pg_htmldoc.whitelist is registered PGC_SUSET, so only a superuser can set
-- it -- confirm that htmldoc_test_none, connected directly (not merely SET
-- ROLE'd) and thus genuinely non-superuser, can't loosen its own scope with
-- a plain SET.
--
ALTER ROLE htmldoc_test_none SET pg_htmldoc.whitelist = 'file:///etc/hostname';
\c - htmldoc_test_none
...
--
-- pg_htmldoc.so isn't actually loaded into this fresh backend yet -- that
-- only happens on the first call into it, below -- so pg_htmldoc.whitelist
-- is still just an unenforced placeholder GUC at this point, not yet
-- reconciled with the PGC_SUSET definition pg_whitelist_init() registers.
-- The PGC_SUSET check further down deliberately runs after this, once the
-- module (and the real GUC definition) is guaranteed to be loaded.
--
SELECT htmldoc_addfile('/etc/hostname');
SELECT octet_length(convert2pdf()) > 100 AS whitelist_reconnect_pdf_nonempty;

DO $$
BEGIN
    BEGIN
        PERFORM set_config('pg_htmldoc.whitelist', 'file:///etc/passwd', false);
        RAISE EXCEPTION 'htmldoc_test_none was able to loosen its own pg_htmldoc.whitelist with a plain SET';
...
-- require_superuser() in every one of them, so this isolates the
-- NULL check from the permission check. convert2pdf/convert2ps need
-- a document already queued to reach their own NULL check instead of
-- the (separate, untested) "no document queued" error, hence the
-- addhtml() in between.
--
