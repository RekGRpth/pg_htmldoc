...
-- per-role restriction on top of the pg_read_server_files/
-- pg_execute_server_program checks above: even htmldoc_test_full, which
-- holds both, can be scoped down to specific file:// / http(s):// prefixes.
...
--
ALTER ROLE htmldoc_test_full SET pg_htmldoc.whitelist = 'file:///etc/hostname';
\c - htmldoc_test_full
...
-- it -- confirm that htmldoc_test_full, connected directly (not merely SET
-- ROLE'd) and thus genuinely non-superuser, still can't loosen its own
-- scope with a plain SET. Wrapped in a DO block guarded by version,
-- mirroring the cleanup-on-rollback check earlier in this file, because
-- pre-PG11 htmldoc_test_full is made an actual superuser instead of merely
-- holding the predefined roles (see the role setup at the top of this
-- file), so the plain SET would legitimately succeed there -- a DO block's
-- own output ("DO") stays identical across versions either way, since the
-- mismatch (if any) is what gets raised, not the version check itself.
--
DO $$
BEGIN
    IF current_setting('server_version_num')::int >= 110000 THEN
        BEGIN
            ...
            RAISE EXCEPTION 'htmldoc_test_full was able to loosen its own pg_htmldoc.whitelist with a plain SET';
...
ALTER ROLE htmldoc_test_full SET pg_htmldoc.whitelist = 'file:///etc/ssl/';
\c - htmldoc_test_full
...
--
\c - :pg_htmldoc_test_orig_user
ALTER ROLE htmldoc_test_full SET pg_htmldoc.whitelist = 'https://example.com/';
\c - htmldoc_test_full

SELECT htmldoc_addurl('http://192.0.2.1/forbidden');

\c - :pg_htmldoc_test_orig_user
ALTER ROLE htmldoc_test_full RESET pg_htmldoc.whitelist;
